...
export class ProductComponent implements OnInit {
  ...
  constructor(private service: ProductService, private router: Router, private eventDriverService: EventDriverService) { }

  ngOnInit(): void {
    this.eventDriverService.sourceEvenSubjectObservable.subscribe((actionEvent: ActionEvent) => {
  ...
  onActionEvent($event: ActionEvent) {
    switch ($event.type) {
      ...
      case ProductActionsType.SELECT_PRODUCT:
        console.log('case reached; Payoad: sssss' + $event)
        this.onSelect($event.payload);
        break;
      case ProductActionsType.DELETE_PRODUCT:
        console.log('case reached; Payoad: ' + $event)
